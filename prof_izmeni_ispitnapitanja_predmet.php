...
<?php
    ...
    $handle = dbConnect();
    $result = mysqli_query($handle, "select * from materijal where tip_materijala = 'ispitnapitanja' and sifra_predmet = '".$_GET['sifra']."' order by datum_objave desc");
    ...
    if ($result !== false && mysqli_num_rows($result) > 0)
    {
        while ($row = mysqli_fetch_assoc($result))
        {
            ...
            echo "<p><a href='".$row['fajlputanja']."' target='_blank'>".$row['naslov']."</a> | ".$row['datum_objave']." | Autor: ".$autor_ime." <a href='prof_izmeni_ispitnapitanja_predmet.php?id=".$row['id_materijal']."'>Izmeni</a> <a href='prof_izmeni_ispitnapitanja_predmet.php?obrisi=1&id=".$row['id_materijal']."'>Obrisi</a></p>";
        }
    }
    ...
    if (isset($_GET['obrisi']) && $_GET['obrisi'] == 1)
    {
        ...
        header("Location:prof_izmeni_ispitnapitanja_predmet.php");
    }
    ...
    if (isset($_POST['dodajfajl']))
    {
        ...
        if (strcmp($_FILES['fajl']['name'], "") !== 0)
        {
            ...
            if ($uploadOK) 
            {
                ...
                $result = mysqli_query($handle, "insert into materijal (naslov, fajlputanja, sifra_predmet, tip_materijala, id_nastavnik, datum_objave, vidljiv) values ('".$_POST['naslov']."', '".$target_file."', '".$_GET['sifra']."', 'ispitnapitanja', '".$_SESSION['email']."', CURDATE(), 1)");
                header("Location:prof_izmeni_ispitnapitanja_predmet.php");
